...
class Section(base.SectionBase):
    ...
    @base.returns_single_item(base.ResponseBase)
    def inspect(self, record: str, verify: str = None,
                dump: bool = True,
                **kwargs: base.CommonArgs):
        """Prints values inside of IPNS Record protobuf and its DAG-CBOR Data field.

        Argument record is of file type. This endpoint expects one or several
                files (depending on the command) in the body of the request as
                'multipart/form-data'.
        ...
        +------+-----------------------------------------+
        | Path | Information about the given IPNS record |
        +------+-----------------------------------------+
        """
        if not os.path.isfile(record):
            raise ValueError("Record must be the path to a file.")
        # opts = {"record": record}
        opts = {}
        if verify is not None:
            opts["verify"] = str(verify)
        if dump is not None:
            opts["dump"] = bool(dump)
        ...
        return self._client.request('/name/inspect', args, data=body, headers=headers, decoder='json', **kwargs)
