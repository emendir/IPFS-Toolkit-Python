...
def TransmitData(data, peerID, buffer_size = def_buffer_size):
    """
    ...
    """
    return Transmitter(data, peerID, buffer_size)
...
def ListenForTransmissions(eventhandler):
    """
    ...
    CreateListeningConnection("TransmissionRequests", port)
...
class Listener(threading.Thread):
    ...
    def run(self):
        ...
        conn.close()
        self.sock.close()
        CloseListeningConnection("TransmissionRequests", self.port)
        if print_log:
...
class Transmitter:
    ...
    def __init__(self, data, peerID, buffer_size=def_buffer_size):
        self.data = data
        self.peerID = peerID
        ...
        print(peerID)
        port = CreateSendingConnection(peerID, "TransmissionRequests")
        print(port)
    ...
    def ProcessConfirmationBuffer(self, data):
        ...
        if(data) == "conf".encode("utf-8"):
            ...
            for buffer in self.sent_buffers:
                if buffer[0] == buffer_No:
                    self.sendbuffer(buffer[1], self.their_port)
                    if print_log:
    ...
    def FinishedTransmission(self):
        ...
        CloseSendingConnection(self.peerID, "TransmissionRequests")
