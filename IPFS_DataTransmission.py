...
def ListenForFileTransmissions(listener_name, eventhandler, dir="."):
    def RequestHandler(conv_name, peerID, others_conv_listener):
        ft = FileTransmissionReceiver()
        conv = Conversation()
        ft.Setup(conv, eventhandler, dir)
        conv.Join(conv_name, peerID, others_conv_listener, ft.OnDataReceived)
...
def CreateSendingConnection(peerID, protocol, port=None):
...
def CreateListeningConnectionTCP(protocol, port):
    try:
        ...
        try:
            ...
            print("")
            print(
                "Exception in NetTerm.ReceiveTransmissions.ReceiveTransmissionRequests()")
            print("----------------------------------------------------")
            traceback.print_exc()  # printing stack trace
            print("----------------------------------------------------")
            print("")
...
def CloseListeningConnection(protocol, port):
    IPFS_API.ClosePortForwarding(
        protocol="/x/" + protocol, targetaddress=f"/ip4/127.0.0.1/udp/{port}")


def ListenToBuffers(eventhandler, port=0, buffer_size=def_buffer_size, monitoring_interval=1, status_eventhandler=None):
...
class ListenerTCP(threading.Thread):
    ...
    def __init__(self, eventhandler, port=0, buffer_size=def_buffer_size, monitoring_interval=1, status_eventhandler=None):
...
class Listener(threading.Thread):
    ...
    def Terminate(self):
        ...
        self.terminate = True   # marking the terminate flag as true
        #self.sock.close()
        SendBuffer(bytearray([0]),"127.0.0.1",self.port) # to make the listener's buffer receiving while loop move forwards so that it realises it has o stop
...
def SendBuffer(buffer, addr, port):
...
class Transmitter:
    ...
    def Transmit(self):
        ...
        while position < len(self.data):
            ...
            # -1 to make space for the integrity buffer that gets added by the SendBuffer function
        ...
        if self.sendbuffer("finished transmission".encode("utf-8")):
            # adding the buffer to the list of sent buffers, just in case we have to resend it
...
class TransmissionListener:
    ...
    def __init__(self, peerID, sender_port, buffer_size, eventhandler):
        ...
        self.listener_thread = ListenToBuffers(
        ...
        self.sendbuffer(ToB255No0s(self.trsm_lis_port) +
    ...
    def ProcessTransmissionBuffer(self, data, peerID):
        ...
        if(data == "finished transmission".encode("utf-8")):
            ...
            for buffer in self.buffers:
                if(buffer == bytearray()):
                    self.sendbuffer(ToB255No0s(index) +
        ...
        self.sendbuffer(ToB255No0s(buffer_No) +
